...
def map_to_genome(segment):
    ...
    try:
        ...
        # print("lalala1",segment.reference_name,genome_info, new_ref_id, trans_dir)
        # time.sleep(1)
    except TypeError:
        genome_info = None
        new_ref_id = None
    if genome_info and new_ref_id is not None:
        ...
        if old_end <= len_transcript:
            ...
            # print("lalala2",genome_info_iter, len_transcript,segment)
            # time.sleep(10)
            unlifted += 1
            if options.no_unlift == False:
                UNLIFT.write(segment)
    else:
        # print("lalala3", segment.reference_name, genome_info, new_ref_id, trans_dir)
        # time.sleep(100)
        unlifted += 1
        if options.no_unlift == False:
            UNLIFT.write(segment)
...
if __name__ == "__main__":
    ...
    header['SQ'] = []
    if options.fasta:
        with open(options.output.split("trans2Genome.bam")[0]+"ref.dict", 'w') as output:
            for seq in SeqIO.parse(options.fasta, 'fasta'):
                length = len(seq.seq)
                header['SQ'].append({"SN": seq.id, "LN": length})
                output.write(seq.id + "\t" + str(length) + "\n")
    elif options.dict:
        with open(options.output.split("trans2Genome.bam")[0]+"ref.dict") as input:
            for line in input.readlines():
                line = line.strip().split("\t")
                header['SQ'].append({"SN": line[0], "LN": int(line[1])})
    elif options.ref:
        with pysam.AlignmentFile(options.output.split("trans2Genome.bam")[0]+"ref.dict", 'rb') as input:
            header = input.header
    else:
