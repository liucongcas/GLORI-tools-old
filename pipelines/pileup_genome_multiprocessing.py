	...
	with pysam.AlignmentFile(options.input, 'rb') as samfile, open(tmp_file_name,'a') as tmp_file:
		try:
			...
			for pileupcolumn in samfile.pileup(conting,start=start,stop=stop,max_depth=max_depth,ignore_orphans=False,ignore_overlaps=False,min_base_quality=0,truncate=True):
				...
				for pileupread in pileupcolumn.pileups:
					if pileupread.query_position is not None and pileupread.alignment.query_qualities[pileupread.query_position] >= options.qual:
						...
						if pileupread.alignment.is_reverse:
							...
							# if conting == "hsa-miR-30e-5p_AG_converted" and pileupread.query_position == 6:
						...
						if not_at_end:
							...
							# if chr == "chr1_AG_converted" and pos == 1439282 and pileupread.query_position is not None:
							# 	print(pileupread.query_position, "lalalala", pileupread,'lalalal2',query_name,query_base,query_qual)
							if pileupread.alignment.has_tag("TS") and pileupread.alignment.get_tag("TS") == "-":
								# print(pileupread, "**********3")
								# print(pos, PF_positive_base, PF_positive_A, pileupread.alignment,
								...
								# print("***************TS+", query_name, pileupread.alignment.query_sequence, A_counts,query_base,query_qual)
				...
				# print("length of list_tmp_positive",len(list_tmp_positive),len(list_tmp_negative))
...
--trim-head and --trim-tail. If you want to trim hexmers at this stage, use this option to trim them.
	...
	print(len(RefBins),len(dictRefSeq))
	index = 1
