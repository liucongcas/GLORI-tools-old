...
# Usage: This program is used to call m6A sites of a single sample.
# Input: [pileups]
    ...
    while (line):
        if line.startswith("#"):
            ...
            if len(line) == 5:
                ...
                gene = gene.replace("#", "")
                # print(line,"********la1",total,Alength,int(total)/int(Alength),options.AG_number,float(nonCR),options.gene_CR,int(total)/int(Alength) < options.AG_number,float(nonCR) >= options.gene_CR)
...
def call_m6A(line, col):
    ...
    if AG_col >= options.coverage and A_count_col >= options.count:
        ...
        # print(line, "*********************2")
        # print(AG_col,total_cov_col,AG_col / (total_cov_col + 0.0),options.var_ratio)
        # print(ratio < options.ratio, AG / (total_cov + 0.0) < options.var_ratio,AG_col / (total_cov_col + 0.0) < options.var_ratio)
        if ratio < options.ratio or AG / (total_cov + 0.0) < options.var_ratio or AG_col / (total_cov_col + 0.0) < options.var_ratio:
            ...
            # print(col, A_count_col, AG_col, nonCR, nonCRs,"******************3")
            ...
            if pvalue < options.pvalue:
                ...
                if Signal >= options.signal:
                    ...
                    # print(col, A_count_col, AG_col, nonCR, pvalue, P_format,"******************2")
...
if __name__ == "__main__":
    ...
    # print("parameters:",options.coverage,options.count,options.ratio,options.pvalue,options.signal)
